...
export function ServicesIntro({ overVideo = false }: { overVideo?: boolean }) {
  return (
    <div className="text-center max-w-2xl mx-auto mb-4 md:mb-6">
      ...
      <h2 className={`text-3xl sm:text-4xl md:text-5xl font-heading font-light tracking-wide mb-6 ${overVideo ? 'text-white' : 'text-foreground'}`}>
        Services Offered
      </h2>
      <div className="w-16 h-[1px] bg-gradient-to-r from-transparent via-accent to-transparent mx-auto mb-6" />
      <p className={`text-sm sm:text-base leading-relaxed max-w-xl mx-auto font-light ${overVideo ? 'text-white/90' : 'text-muted-foreground'}`}>
...
export default function Services({ showImages = true, showIntro = true }: ServicesProps) {
  return (
    <section id="services" className="py-4 md:py-6 px-6 sm:px-8 lg:px-10 bg-secondary/20">
          ...
          {brand.services.map((service, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative bg-card border border-border/50 rounded-sm hover:border-accent/70 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between overflow-hidden shadow-cinematic hover:shadow-tan-glow ${!showImages ? 'min-h-[145px]' : 'min-h-[240px]'}`}
              >
                {showImages ? (
